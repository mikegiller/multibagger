# master_plan.py — shared Master Plan logic for Stocks/ETFs and Indexes
#
# The option-chain filtering/optimum-strike logic is identical for both asset
# classes (two-sided-quote requirement, no lastPrice fallback) — only the
# favorites list, page title, and session-state namespace differ per class.
...
def render(asset_class):
    """Render the full Master Plan page for asset_class ("stocks" or "index")."""
    cfg = ASSET_CONFIG[asset_class]

    def sk(name):
        return f"mp_{asset_class}_{name}"

    def render_master_plan(ticker, chains, last_date, linreg, current_price, is_weekly, active_only):
        if linreg is None:
    ...
    _defaults = {
        ...
        sk("horizon"): "3 Months",
        sk("lt_period"): "2y",
        sk("lt_horizon"): "1 Year",
    }
    for key, v in _defaults.items():
        if key not in st.session_state:
            st.session_state[key] = v

    if "fav" in st.query_params and st.query_params["fav"] in _favs:
        st.session_state[sk("ticker")] = st.query_params["fav"]
    ...
    with st.sidebar:
        st.header("Settings")
        if st.button("Clear Cache & Refresh", key=sk("clear_cache_btn")):
            st.cache_data.clear()
            st.rerun()

    # === Shared Ticker Input ===
    _fav_placeholder = st.empty()
    ticker = utils.ticker_input(sk("ticker"), category=cfg["favorites_category"], max_chars=12)
    if not ticker:
        st.stop()

    utils.warn_if_wrong_report(
        ticker, asset_class,
        stocks_page="pages/Master_Plan_Stocks.py",
        index_page="pages/Master_Plan_Index.py",
    )
    ...
    with tab_st:
        ...
        for i, (period_key, lbl) in enumerate(ST_PERIOD_OPTS.items()):
            with _st_per_cols[i]:
                st.button(lbl, key=sk(f"stper_{period_key}"), width='stretch',
                          on_click=lambda _k=period_key: st.session_state.update({sk("period"): _k}))
                pct = period_changes.get(period_key)
                if pct is not None:
                    st.markdown(pct_label(pct), unsafe_allow_html=True)
        period = st.session_state[sk("period")]
        ...
        for i, (horizon_key, bars) in enumerate(ST_HORIZON_OPTS.items()):
            with _st_hor_cols[i]:
                st.button(horizon_key, key=sk(f"sthor_{horizon_key.replace(' ','_')}"), width='stretch',
                          on_click=lambda _k=horizon_key: st.session_state.update({sk("horizon"): _k}))
        horizon_label = st.session_state[sk("horizon")]
        ...
        if df is None or df.empty:
            ...
            with _oc[0]:
                ...
                show_lr_center = st.checkbox("LR Center",    value=True,  key=sk("lr_center"))
                show_lr_1std   = st.checkbox("LR ±1σ",       value=False, key=sk("lr_1std"))
                show_lr_2std   = st.checkbox("LR ±2σ",       value=False, key=sk("lr_2std"))
            with _oc[1]:
                st.caption("Trend Lines")
                show_tl_resist  = st.checkbox("Resistance",  value=False, key=sk("tl_resist"))
                show_tl_support = st.checkbox("Support",     value=False, key=sk("tl_support"))
            with _oc[2]:
                st.caption("Moving Averages")
                ma_20  = st.checkbox("SMA 20",  value=False, key=sk("ma20"))
                ma_50  = st.checkbox("SMA 50",  value=False, key=sk("ma50"))
                ma_100 = st.checkbox("SMA 100", value=False, key=sk("ma100"))
                ma_200 = st.checkbox("SMA 200", value=False, key=sk("ma200"))
            with _oc[3]:
                st.caption("Fibonacci")
                show_fib = st.checkbox("Fib Retracement", value=False, key=sk("fib"))
            with _oc[4]:
                st.caption("Bollinger (20)")
                show_bb_bands  = st.checkbox("BB Upper/Lower", value=False, key=sk("bb_bands"))
                show_bb_center = st.checkbox("BB Center",      value=False, key=sk("bb_center"))
            with _oc[5]:
                st.caption("S/R Horizontal")
                show_sr_resist  = st.checkbox("S/R Resistance", value=False, key=sk("sr_resist"))
                show_sr_support = st.checkbox("S/R Support",    value=False, key=sk("sr_support"))
            with _oc[6]:
                st.caption("Options Filter")
                active_only = st.checkbox("Active only", value=True, key=sk("active_only"),
            ...
            render_master_plan(ticker, call_chains, df.index[-1], linreg_data, current_price,
    ...
    with tab_lt:
        ...
        for i, (period_key, lbl) in enumerate(LT_PERIOD_OPTS.items()):
            with _lt_per_cols[i]:
                st.button(lbl, key=sk(f"ltper_{period_key}"), width='stretch',
                          on_click=lambda _k=period_key: st.session_state.update({sk("lt_period"): _k}))
                pct = lt_period_changes.get(period_key)
                if pct is not None:
                    st.markdown(pct_label(pct), unsafe_allow_html=True)
        lt_period = st.session_state[sk("lt_period")]

        # --- Projection Horizon ---
        LT_HORIZON_OPTS = {"6 Months": 26, "1 Year": 52, "2 Years": 104}
        st.write("**Select projection horizon:**")
        _lt_hor_cols = st.columns(len(LT_HORIZON_OPTS))
        for i, (horizon_key, bars) in enumerate(LT_HORIZON_OPTS.items()):
            with _lt_hor_cols[i]:
                st.button(horizon_key, key=sk(f"lthor_{horizon_key.replace(' ','_')}"), width='stretch',
                          on_click=lambda _k=horizon_key: st.session_state.update({sk("lt_horizon"): _k}))
        lt_horizon_label = st.session_state[sk("lt_horizon")]
        ...
        if df_lt is None or df_lt.empty:
            ...
            with _oc_lt[0]:
                st.caption("Linear Regression")
                lt_lr_center = st.checkbox("LR Center",    value=True,  key=sk("lt_lr_center"))
                lt_lr_1std   = st.checkbox("LR ±1σ",       value=False, key=sk("lt_lr_1std"))
                lt_lr_2std   = st.checkbox("LR ±2σ",       value=False, key=sk("lt_lr_2std"))
            with _oc_lt[1]:
                st.caption("Trend Lines")
                lt_tl_resist  = st.checkbox("Resistance",  value=False, key=sk("lt_tl_resist"))
                lt_tl_support = st.checkbox("Support",     value=False, key=sk("lt_tl_support"))
            with _oc_lt[2]:
                st.caption("Weekly MAs")
                lt_ma13  = st.checkbox("SMA 13w  (~1Q)",  value=False, key=sk("lt_ma13"))
                lt_ma26  = st.checkbox("SMA 26w  (~6M)",  value=False, key=sk("lt_ma26"))
                lt_ma52  = st.checkbox("SMA 52w  (~1Y)",  value=False, key=sk("lt_ma52"))
                lt_ma104 = st.checkbox("SMA 104w (~2Y)",  value=False, key=sk("lt_ma104"))
            with _oc_lt[3]:
                st.caption("Fibonacci")
                lt_fib = st.checkbox("Fib Retracement", value=False, key=sk("lt_fib"))
            with _oc_lt[4]:
                st.caption("Bollinger (26w)")
                lt_bb_bands  = st.checkbox("BB Upper/Lower", value=False, key=sk("lt_bb_bands"))
                lt_bb_center = st.checkbox("BB Center",      value=False, key=sk("lt_bb_center"))
            with _oc_lt[5]:
                st.caption("S/R Horizontal")
                lt_sr_resist  = st.checkbox("S/R Resistance", value=False, key=sk("lt_sr_resist"))
                lt_sr_support = st.checkbox("S/R Support",    value=False, key=sk("lt_sr_support"))
            with _oc_lt[6]:
                st.caption("Options Filter")
                lt_active_only = st.checkbox("Active only", value=True, key=sk("lt_active_only"),
                                             help="Volume > 0 or OI > 10, traded in last 30 days")
            ...
            st.markdown("---")
            st.subheader(f"Master Plan — Optimum Call by Expiration ({ticker})")
            render_master_plan(ticker, call_chains, df_lt.index[-1], lt_linreg, current_price_lt,
